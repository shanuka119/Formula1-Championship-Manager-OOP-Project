...
public class AscendingOrder implements ActionListener {
    //Creating the new GUI frame with ascending order table
    ...
    AscendingOrder(){
        ...
        frame.setTitle("Ascending Order Table");
        ...
        for (Formula1Driver formula1Driver: Formula1ChampionshipManager.ascendingOrderArrayList){
            Object[] rows = {formula1Driver.getDriversName(), formula1Driver.getDriversTeam(), formula1Driver.getDriversLocation(), formula1Driver.getRacesParticipated(),
                    formula1Driver.getTotalPoints(), formula1Driver.getFirstPositions(), formula1Driver.getSecondPositions(), formula1Driver.getThirdPositions()};
            table.addRow(rows);
